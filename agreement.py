...
if __name__ == "__main__":
    ...
    # lx.add('duck','N')
    # #lx.add('')
    # tr0 = all_valid_parses(lx, ['Which', 'duck', 'eats','?'])[0]
    # tr = restore_words(tr0, ['Which', 'duck', 'eats','?'])
    # tr.draw()
    lx = Lexicon()
    lx.add('cute','A')
    lx.add('Megan','P')
    lx.add('bird','N')
    lx.add('fight','I')
    tr0 = all_valid_parses(lx, ['Which','cute','bird','does','Megan','fight','?'])[0]
    tr = restore_words(tr0,['Which','cute','bird','does','Megan','fight','?'])
    print(str(tr0))
    print(str(tr))
