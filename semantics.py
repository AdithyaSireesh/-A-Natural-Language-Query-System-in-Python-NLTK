...
def sem(tr):
    ...
    # add code here
        ...
        return '(\\x.(' + sem(tr[2]) + '(x) & ' + sem(tr[1]) + '(x)))'
    elif (rule == 'QP -> VP'):
        return '(\\x.(' + sem(tr[0]) + '(x)))'
    elif (rule == 'QP -> DO NP T'):
        return '(\\x.(exists y.(' + sem(tr[2]) + '(y) & ' + sem(tr[1]) + '(x))))'
    elif (rule == 'VP -> I'):
        return '(\\x.(' + sem(tr[0]) + '(x)))'
    elif (rule == 'VP -> BE A'):
        return '(\\x.(' + sem(tr[1]) + '(x)))'
    elif (rule == 'VP -> T NP'):
        return '(\\x.(exists y.(' + sem(tr[1]) + '(x) & ' + sem(tr[0]) +'(x))))'
    elif (rule == 'VP -> BE NP'):
        return '(\\x.(' + sem(tr[1]) + '(x)))'
        ...
        return '(\\x.(' + sem(tr[1]) + '(x) & ' + sem(tr[0]) + '(x)))'
    elif (rule == 'AN -> N'):
        return '(\\x.(' + sem(tr[0]) + '(x)))'
    elif (rule == 'Rel -> WHO VP'):
        return '(\\x.(' + sem(tr[1]) + '(x)))'
    elif (rule == 'Rel -> NP T'):
        return '(\\x.(exists y.(' + sem(tr[1]) + ' & ' + sem(tr[0]) + '(x))))'
    else:
        return
    # add more code here
